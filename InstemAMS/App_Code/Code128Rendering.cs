...
namespace GenCode128
{
	...
	public static class Code128Rendering
	{
      ...
      public static Image MakeBarcodeImage( string InputData, double BarWeight, bool AddQuietZone )
      {
         ...
         double width, height;
         width = Math.Round(((codes.Length - 3) * 11 + 35) * BarWeight);
        ...
        Bitmap myimg = new Bitmap(Convert.ToInt32(width), Convert.ToInt32(height));
         myimg.SetResolution(96.0F, 96.0F);
         //Image myimg = new System.Drawing.Bitmap(width, height);
        // Image myimg = new System.Drawing.Image(bitmap);
          ...
          using (Graphics gr = Graphics.FromImage(myimg))
         {
            ...
            // set to white so we don't have to fill the spaces with white
             gr.FillRectangle(System.Drawing.Brushes.White, 0, 0, Convert.ToInt32(width), Convert.ToInt32(height));

            // skip quiet zone
            double cursor = AddQuietZone ? cQuietWidth * BarWeight : 0;
            ...
            for (int codeidx = 0; codeidx < codes.Length; codeidx++)
            {
               ...
               {
                  double barwidth = cPatterns[code, bar] * BarWeight;
                  double spcwidth = cPatterns[code, bar + 1] * BarWeight;

                  // if width is zero, don't try to draw it
                  if (barwidth > 0)
                  {
                     gr.FillRectangle(System.Drawing.Brushes.Black, Convert.ToInt32(cursor), 0, Convert.ToInt32(barwidth), Convert.ToInt32(height));
                  }
