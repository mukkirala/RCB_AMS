...
public partial class ViewAllReuestApprover : System.Web.UI.Page
{
    ...
    {
        string select1 = "select AssetRequestID as RequestID,RequestType,AssetTypeName as AssetName,Date as RequestedDate,EmployeeAssetRequest.Status from EmployeeAssetRequest inner join InstemSAP..AssetTypeMaster on AssetTypeMaster.AssetTypeID = EmployeeAssetRequest.AssetTypeID where RequestType!='' and ApproverID='" + Session["UserID"] + "'";
        SqlCommand cmd1 = new SqlCommand(select1, conAMS);
        SqlDataAdapter da1 = new SqlDataAdapter(cmd1);
        DataTable datatable1 = new DataTable();
        da1.Fill(datatable1);

        string select2 = "select BuybackRequestID as RequestID,RequestType,AssetMaster.AssetDesc as AssetName,Date as RequestedDate,EmployeeAssetBuyback.Status from EmployeeAssetBuyback inner join InstemSAP..AssetMaster on AssetMaster.AssetID = EmployeeAssetBuyback.AssetID where RequestType!='' and ApproverID='" + Session["UserID"] + "'";
        SqlCommand cmd2 = new SqlCommand(select2, conAMS);
        SqlDataAdapter da2 = new SqlDataAdapter(cmd2);
        DataTable datatable2 = new DataTable();
        da2.Fill(datatable2);

        string select3 = "select LocationChangeID as RequestID,RequestType,AssetMaster.AssetDesc as AssetName,Date as RequestedDate,EmployeeLocationChange.Status from EmployeeLocationChange inner join InstemSAP..AssetMaster on AssetMaster.AssetID=EmployeeLocationChange.AssetID where RequestType!=''  and ApproverID = '" + Session["UserID"] + "'";
        SqlCommand cmd3 = new SqlCommand(select3, conAMS);
        SqlDataAdapter da3 = new SqlDataAdapter(cmd3);
        DataTable datatable3 = new DataTable();
        da3.Fill(datatable3);

        string select4 = "select CustodianChangeID as RequestID,RequestType,AssetMaster.AssetDesc as AssetName,Date as RequestedDate,CustodianChangeRequest.Status from CustodianChangeRequest inner join InstemSAP..AssetMaster on AssetMaster.AssetID = CustodianChangeRequest.AssetID where RequestType!='' and ApproverID='" + Session["UserID"] + "'";
