    ...
    public void ProcessRequest (HttpContext context) {
        if (context.Request.Files.Count > 0)
        {
            ...
            for (int i = 0; i < files.Count; i++)
            {
                ...
                fname=context.Server.MapPath(@"~/Images/"+ filenam1+extension);
                //if (context.Session["filename"] != null)
                //fname = context.Session["filename"].ToString();

                HttpContext.Current.Session["imgname"]=fname;
                string filename = @"~/Images/" + filenam1+extension;
                HttpContext.Current.Session["imgpath"] = filename;
                file.SaveAs(fname);
            }
        }
    }
